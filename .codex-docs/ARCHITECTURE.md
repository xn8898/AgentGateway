...
IMtoAgent 的核心架构哲学：**中间厚，上下薄**。

```
    ┌─────────────────────────────────────┐
    │   上层：超薄 IM 适配器               │  ← 只翻译消息格式
    │   Feishu / Telegram / WeCom          │    只管收发 + 声明能力
    ├─────────────────────────────────────┤
    │                                     │
    │   中层：厚核心（路由中枢）             │  ← 所有业务逻辑集中在这里
    │   • IM Registry 工厂                 │
    │   • 消息路由 & Bot 分发              │
    │   • 命令体系                          │
    │   • Prompt Builder + Soul 注入       │
    │   • 能力解析 (resolveCapabilities)   │
    │   • 输出翻译 (parseToBlocks)         │
    │   • SDK Core (AgentRuntime/Session)  │
    │   • 统一 Proxy (:18899)             │
    │                                     │
    ├─────────────────────────────────────┤
    │   下层：超薄 Agent 适配器             │  ← 只翻译协议
    │   Claude / Codex / OpenCode          │    只管对接 + 按标准接口交付
    └─────────────────────────────────────┘
```

### 核心原则

**上层薄：** IM 适配器只做最基础的翻译工作。

每个 IM 适配器唯一要关心的事：
1. **接收** → 把外部消息事件转成内部格式，交给路由
2. **发送** → 把内部格式转回该 IM 的原生格式
3. **能力声明** → `getCapabilities()` 告诉上层"我能做什么"

IM 模块**不关心**：
- 对面是哪个 Agent（Claude / Codex / OpenCode）
- Prompt 怎么构建
- 模型请求怎么代理
- 会话怎么管理

**下层薄：** Agent 适配器只做最基础的对接工作。

每个 Agent 适配器唯一要关心的事：
1. **接收** → 按该 Agent 的协议格式调用后端
2. **返回** → 把后端响应转成标准格式
3. **会话** → 按 `AgentAdapter` 接口交付
...
中间层是**唯一**需要变聪明的地方：
- 路由：消息该给哪个 Bot、哪个会话
- Prompt 构建：Soul 注入 + IM 能力声明 → 系统提示词
- 输出翻译：Agent 的 markdown → `parseToBlocks()` → IM 原生渲染
- 会话管理：持久化、统计、错误处理
- 模型代理：协议转换、格式翻译
...
| 场景 | 工作量 | 改哪里 |
|------|--------|--------|
| 新增 IM 渠道（如钉钉） | 写一个薄适配器，实现 `IMModule` 接口 | `modules/im/dingtalk.ts` + Registry 加一行 |
| 新增 Agent 后端（如 ChatGPT） | 写一个薄适配器，实现 `AgentAdapter` 接口 | `modules/agent/chatgpt-adapter.ts` |
| 改 Prompt 逻辑 | 只改中间层 | `modules/prompt-builder.ts` |
| 改输出解析 | 只改中间层 | `modules/capabilities.ts` |

新增渠道/后端，工作量基本就是"翻译收发消息格式"。中间层不需要改。

### 设计收益

- **可测试性**：上下层薄，边界清晰，单元测试容易
- **可维护性**：业务逻辑集中在中间，改了不扩散
- **可扩展性**：新增 IM 或 Agent 不改核心，接口即合同
- **团队分工**：懂飞书的写飞书模块，懂 Claude 的写 Claude 模块，各自独立
...
| Codex | ✅ :18899（统一端口） Responses ↔ Chat Completions | exec-server WebSocket（优先）+ exec CLI 回退 | ✅ |
...
飞书消息 → IMtoAgent (Bun)
              ...
              └─ Bot "CodexBot"   → codex exec/resume  → :18899 → DeepSeek API
```
...
| `codex-proxy.ts` | Codex 请求处理：Responses API ↔ Chat Completions 转换（导出 handleCodexRequest，由 :18899 统一端口调度） |
