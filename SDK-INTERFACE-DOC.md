...
- [4. IMModule — IM 平台适配器（规划中）](#4-imadapter--im-平台适配器规划中)
- [5. 核心类型定义](#5-核心类型定义)
...
| 层级 | 组件 | 职责 |
|------|------|------|
| IM 层 | `IMModule` (规划中) | 连接 IM 平台，收发消息，声明输出能力 |
...
```
用户消息 → IMModule → Bot.handleMessage()
...
> ✅ 接口已标准化为 `IMModule`（`modules/types.ts`）。`FeishuIMModule` 实现了该接口。
> `Bot` 类通过 `im: IMModule` 字段使用，不依赖具体平台。新平台只需实现 `IMModule` 接口即可接入。
